...
public class HeroGenerator {
    ...
    private static void generateHero(Scanner scanner, List<Hero> heroes) {
        ...
        if (!isValidRace(race) || !isValidType(classe)) {
            System.out.println("Entrée invalide. Veuillez réessayer.");
            return;
        }

        // Définir la vitesse en fonction de la race
        int speed = switch (race) {
            case "humain" -> 4;
            case "elfe" -> 5;
            case "nain" -> 3;
            default -> {
                System.out.println("Race invalide, la vitesse par défaut sera définie à 3.");
                yield 3;
            }
        };
        ...
        Hero hero = new Hero(classe, race, speed);
        hero.setColor(color); // Assurez-vous que Hero a une méthode setClasseCouleur()
        heroes.add(hero); // Ajouter le héros généré à la liste

        System.out.println("Héros généré avec succès !");
        System.out.println("Classe : " + hero.getClasse());
